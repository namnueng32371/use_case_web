...
LISTS = {
    ...
    "V": ("AI ประมวลผลกี่รายการ/วัน", [
        ("น้อยกว่า 100 รายการ  (ชิ้น / ล็อต / คำถาม / เคส)", None),
        ("100 - 1,000 รายการ  (ชิ้น / ล็อต / คำถาม / เคส)", None),
        ("1,000 - 10,000 รายการ  (ชิ้น / ล็อต / คำถาม / เคส)", None),
        ("10,000 - 100,000 รายการ  (ชิ้น / ล็อต / คำถาม / เคส)", None),
        ("มากกว่า 100,000 รายการ  (ชิ้น / ล็อต / คำถาม / เคส)", None)]),
    ...
    "AC": ("ต้องเทรนโมเดลใหม่บ่อยแค่ไหน", [
        ...
        ("ยังไม่แน่ใจ", None)]),
    "AE": ("งานนี้เริ่มทำงานยังไง", [
        ("คนกดใช้เอง - มีคนเปิดใช้เป็นครั้งๆ", None),
        ("ทำงานอัตโนมัติต่อเนื่อง - ระบบสั่งเอง ไม่มีคนกด", None),
        ("รันตามรอบเวลา - เช่น ทุกคืน ทุกเช้าวันจันทร์", None),
        ("ทำงานเมื่อมีเหตุการณ์ - เช่น มีใบ NC เข้ามา", None)]),
...
cols1 = [
    ...
    ("ฝ่าย / แผนก", 16, None),
    ("ผู้รับผิดชอบ\n(Owner)", 14, None),
    ("QWP อ้างอิง\n(รหัส / ชื่อเอกสาร)", 28, None),
    ("kQI / QI ที่เกี่ยวข้อง", 26, None),
    ("ขั้นตอนที่เกี่ยวข้อง\n(ขั้นตอนใน SQP ที่ use case นี้แตะ)", 34, None),
...
groups2 = [
    ...
    ("โซน A : ปริมาณงาน  (แผนกกรอก)", 6, FILL_IMPACT),
...
IT_ZONE = range(18, 24)   # คอลัมน์ R-W : โซน B ที่ทีม AI/IT เติม
ex2 = [1, "Auto Inspection ตรวจคุณภาพชิ้นงานด้วย Machine Learning", "Production",
       "ทำงานอัตโนมัติต่อเนื่อง - ระบบสั่งเอง ไม่มีคนกด",
       "รูปภาพ", "1 - 10 คน", "1 - 2 คน", "1,000 - 10,000 รายการ  (ชิ้น / ล็อต / คำถาม / เคส)", "2 - 3 จุด",
...
build_sheet(
    ...
    prompts={
        "D": "ตอบก่อนช่องอื่น เพราะเป็นตัวบอกว่าช่องที่เหลือต้องกรอกยังไง\n\n"
             "งานที่ระบบสั่งเอง จะไม่มี 'คนใช้งาน' — ให้กรอกคนที่เข้ามาดูผลแทน",
        "F": "นับคนทั้งหมดที่เข้ามาใช้ตลอดทั้งวัน ไม่ใช่พร้อมกัน\n\n"
             "ถ้าเป็นงานอัตโนมัติหรือรันตามรอบ ให้นับคนที่เข้ามาดูผล",
        "G": "ช่วงที่คนใช้เยอะที่สุดของวัน มีกี่คนกดพร้อมกัน\n\n"
             "ต้องไม่เกินจำนวนคนที่ใช้งาน/วัน\n"
             "งานอัตโนมัติ/รันตามรอบ มักเป็น 1 - 2 คน\n\n"
             "ถ้าขึ้นพื้นสีแดง = ตัวเลขดูสูงผิดปกติ ลองทบทวนอีกครั้ง",
        "H": "นับรายการที่ AI ต้องประมวลผลจริง ไม่ใช่จำนวนครั้งที่สั่งงาน\n\n"
             "คนกดใช้เอง = กี่คำถาม\n"
             "อัตโนมัติ = กี่ชิ้น กี่ภาพ\n"
             "รันตามรอบ = กี่ล็อต กี่แถวในรอบนั้น\n"
             "เมื่อมีเหตุการณ์ = กี่เคส\n\n"
...
for i, u in enumerate(o for o, _ in LISTS["T"][1]):
    ...
    if over:
        cond = f'$G{FIRST}="' + f'",$G{FIRST}="'.join(over) + '"'
        _pairs.append(f'AND($F{FIRST}="{u}",OR({cond}))')
