...
LISTS = {
    "A": ("บริษัท", [("S.K.Polymer", None), ("Thairubbtech", None), ("Polymate", None),
                     ("GTK", None), ("อื่นๆ", None)]),
    "B": ("รูปแบบข้อมูลที่มี", [
        ("กระดาษ / เอกสารเขียนมือ", 1), ("ไฟล์ Excel / CSV", 4),
        ("ฐานข้อมูลในระบบ (ERP/MES/SQL)", 5), ("รูปภาพ / ภาพถ่าย", 3),
        ("เสียง / วิดีโอ", 2), ("ยังไม่มีข้อมูลเก็บไว้", 1)]),
    "C": ("ข้อมูลย้อนหลัง", [("ยังไม่มี", 1), ("น้อยกว่า 1 ปี", 2),
                             ("1 - 3 ปี", 4), ("มากกว่า 3 ปี", 5)]),
    ...
    "E": ("มีระบบเดิมที่ต้องเชื่อมต่อ", [("ไม่มี - ทำระบบใหม่แยกได้เลย", 5),
                                        ("มี - เชื่อมแบบส่งไฟล์/Export ได้", 3),
                                        ("มี - ต้องเชื่อมตรงกับระบบหลัก", 1),
                                        ("ยังไม่แน่ใจ", 2)]),
    "F": ("มีคนดูแลต่อ", [("มี - ระบุชื่อในหมายเหตุ", 5), ("ยังไม่แน่ใจ", 3), ("ไม่มี", 1)]),
    "G": ("ผลกระทบถ้า AI ผิด", [("ต่ำ - แก้ไขได้ทันที ไม่กระทบใคร", 1),
                                ("กลาง - เสียเวลา/ต้นทุนบางส่วน", 3),
                                ("สูง - กระทบลูกค้า/ความปลอดภัย/กฎหมาย", 5)]),
    "H": ("ต้องมีคนตรวจซ้ำ", [("ไม่ต้อง - เชื่อผล AI ได้เลย", 1),
                              ("สุ่มตรวจบางส่วน", 2), ("ตรวจซ้ำทุกครั้ง", 4)]),
    "I": ("ระดับความลับข้อมูล", [("สาธารณะ - ไม่มีข้อมูลลับ", 1),
                                 ("ข้อมูลภายใน - ห้ามออกนอกบริษัท", 2),
                                 ("ลับ - ต้องมี Access Control", 4),
                                 ("ลับมาก - Regulatory / PDPA", 5)]),
...
ws_d["S1"] = "ค่าตั้งต้นของการคำนวณคะแนน (แก้ได้ ทุกสูตรจะเปลี่ยนตาม)"
ws_d["S1"].font = Font(name=FONT, size=11, bold=True, color="C00000")
for cell, lbl in [("S3", "น้ำหนักแต่ละปัจจัย")]:
    put(cell, lbl, F_HEAD)
for r, (lbl, val) in enumerate([("Impact (ผลกระทบ)", 0.35), ("Feasibility (ความเป็นไปได้)", 0.25),
    ...
    put(f"S{r}", lbl)
    put(f"T{r}", val, F_BLUE, "0%")
put("S9", "เกณฑ์แบ่งระดับ", F_HEAD)
put("S10", "คะแนน >= นี้ = สูง");   put("T10", 75, F_BLUE)
put("S11", "คะแนน >= นี้ = กลาง"); put("T11", 50, F_BLUE)

put("S14", "หมายเหตุ", F_HEAD)
put("S15", "ช่วงชั่วโมง/สัปดาห์ เปลี่ยนเป็น dropdown แล้ว")
put("S16", "แก้ตัวเลือกและคะแนนได้ที่คอลัมน์ R")

put("S23", "ช่วงมูลค่าความเสียหาย/ปี -> คะแนน", F_HEAD)
put("S24", "ตั้งแต่ (บาท)", F_HEAD); put("T24", "คะแนน", F_HEAD)
for i, (th, sc) in enumerate([(0, 1), (1, 2), (100000, 3), (500000, 4), (2000000, 5)], start=25):
    put(f"S{i}", th, F_BLUE, "#,##0"); put(f"T{i}", sc)
ws_d.column_dimensions["S"].width = 30
ws_d.column_dimensions["T"].width = 12

VALUE_TH = "DATA!$S$25:$S$29"
...
cols1 = [
    ...
    ("มูลค่าความเสียหาย/ของเสีย\nต่อปี (บาท) - ถ้าประเมินได้", 18, None),
    ("ข้อมูลที่มีอยู่ตอนนี้\nData Available\n(มีอะไร เก็บที่ไหน ใครเก็บ)", 38, None),
    ("รูปแบบข้อมูล", 24, "B"),
    ("ข้อมูลย้อนหลัง", 16, "C"),
    ("ปริมาณข้อมูลโดยประมาณ\n(เช่น 5,000 รูป / 2 ปี)", 18, None),
    ("สิทธิ์นำข้อมูลมาใช้\n(เจ้าของข้อมูลยอมให้เอามาพัฒนา AI มั้ย)", 34, "D"),
    ("มีระบบเดิมที่ต้องเชื่อมต่อ", 26, "E"),
    ("มีคนในแผนกดูแลต่อได้มั้ย", 22, "F"),
    ("ผลกระทบถ้า AI ทำผิด", 26, "G"),
    ("ต้องมีคนตรวจซ้ำมั้ย", 22, "H"),
    ("ระดับความลับของข้อมูล", 26, "I"),
    ("ความสำคัญที่แผนกอยากได้", 22, "J"),
    ("ต้องการใช้งานภายใน\n(ไตรมาส/ปี)", 16, None),
    ("หมายเหตุ", 26, None),
]
...
ex1 = [1, "Auto Inspection ตรวจคุณภาพชิ้นงานด้วย Machine Learning",
       ...
       "มากกว่า 40 ชม./สัปดาห์  (มากกว่า 1 คนเต็มเวลา)", 850000,
       "มีรูปถ่ายชิ้นงานดี/เสียเก็บไว้ในมือถือหัวหน้ากะ ประมาณ 2 ปี และใบบันทึกผลตรวจเป็นกระดาษ",
       "รูปภาพ / ภาพถ่าย", "1 - 3 ปี", "ประมาณ 4,000 รูป (ดี 3,000 / เสีย 1,000)",
       "ได้ทันที - ข้อมูลอยู่ในมือแผนกเราเอง",
       "ไม่มี - ทำระบบใหม่แยกได้เลย", "มี - ระบุชื่อในหมายเหตุ",
       "สูง - กระทบลูกค้า/ความปลอดภัย/กฎหมาย", "สุ่มตรวจบางส่วน",
...
cols2 = [
    ...
    ("จำนวน Users/วัน", 14, None),
    ("Peak concurrent\n(คนใช้พร้อมกันสูงสุด)", 16, None),
    ("Requests/วัน\n(ประมาณ)", 14, None),
    ("ขนาดข้อมูล Input/วัน\n(GB)", 16, None),
    ("ขนาดคลังเอกสารทั้งหมด (GB)\n(สำหรับงานค้นหา/RAG)", 20, None),
    ("จำนวนรูป/วัน\n(งานภาพเท่านั้น)", 14, None),
    ("ความละเอียดรูป", 18, "O"),
    ("ภาษาที่ต้องการ", 18, "L"),
...
ex2 = [1, "Auto Inspection ตรวจคุณภาพชิ้นงานด้วย Machine Learning", "Production",
       "รูปภาพ", 5, 2, 600, 3, 0, 600, "1 - 5 MP", "ไม่เกี่ยวกับภาษา",
...
for r in [6] + list(range(FIRST, LAST + 1)):
    ...
    ws3.cell(row=r, column=5).value  = F(f'IF({S1}!J{r}="","",IFERROR(MATCH({S1}!J{r},{VALUE_TH},1),1))')
    # ถ้ากรอกไม่ครบ ใช้ตัวที่มีตัวเดียว ถ้าไม่มีเลยค่อยเว้นว่าง
    ws3.cell(row=r, column=6).value  = F(
        f'IF(D{r}="",IF(E{r}="","",E{r}),IF(E{r}="",D{r},ROUND((D{r}+E{r})/2,0)))')
    # Data
    ...
    miss = "+".join(f'COUNTBLANK({S1}!{c}{r})' for c in "ILMOPQRST")
    ws3.cell(row=r, column=21).value = F(miss)
    # รวม — ถ้ากรอกไม่ครบ ไม่แสดงคะแนน
    ws3.cell(row=r, column=18).value = F(
        f'IF(U{r}>0,"",IFERROR(ROUND((F{r}*DATA!$T$4+M{r}*DATA!$T$5+J{r}*DATA!$T$6'
        f'+(6-Q{r})*DATA!$T$7)*20,1),""))')
    ws3.cell(row=r, column=19).value = F(
        f'IF(U{r}>0,"ข้อมูลไม่ครบ",'
        f'IF(R{r}="","",IF(R{r}>=DATA!$T$10,"สูง",IF(R{r}>=DATA!$T$11,"กลาง","ต่ำ"))))')
    if r == 6:
...
for i, txt in enumerate([
    ...
    "ช่องบังคับ 9 ช่อง = เวลาที่ใช้ / รูปแบบข้อมูล / ข้อมูลย้อนหลัง / สิทธิ์ใช้ข้อมูล / ระบบเดิม / "
    "คนดูแลต่อ / ผลกระทบถ้า AI ผิด / การตรวจซ้ำ / ระดับความลับ   (มูลค่าความเสียหายไม่บังคับ)",
