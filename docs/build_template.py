...
LISTS = {
    ...
    "Z": ("ติดตั้งแบบไหน", [
        ...
        ("ยังไม่แน่ใจ", None)]),
    "AA": ("มีเครื่อง/GPU เดิมใช้ได้มั้ย", [
        ("มี GPU ว่างอยู่ - ใช้ได้เลย", None),
        ("มี server แต่ไม่มี GPU", None),
        ("ไม่มีเลย - ต้องซื้อใหม่ทั้งหมด", None),
        ("ยังไม่แน่ใจ - ต้องถาม IT", None)]),
    ...
    "AC": ("ต้องเทรนโมเดลใหม่บ่อยแค่ไหน", [
        ...
        ("ปีละครั้ง", None), ("ทุกไตรมาส", None),
        ("ทุกเดือนหรือถี่กว่า", None), ("ยังไม่แน่ใจ", None)]),
    "AD": ("แชร์เครื่องกับงานอื่นได้มั้ย", [
        ("แชร์ได้ - ไม่ซีเรียสเรื่องความเร็ว", None),
        ("แชร์ได้บางส่วน - ช่วงพีคต้องได้ก่อน", None),
        ("แชร์ไม่ได้ - ต้องมีเครื่องของตัวเอง", None),
        ("ยังไม่แน่ใจ", None)]),
}
...
groups2 = [("ลิงก์อัตโนมัติจากชั้นที่ 1", 3, FILL_BASIC), ("ปริมาณงาน (Workload)", 6, FILL_IMPACT),
           ("ความต้องการเชิงเทคนิค", 6, FILL_TECH),
           ("การติดตั้งและใช้เครื่องร่วมกัน", 4, FILL_FEAS),
           ("ข้อจำกัด / งบประมาณ", 4, FILL_PLAN)]
cols2 = [
    ("No.", 6, None),
    ...
    ("ต้องเทรนโมเดลใหม่บ่อยแค่ไหน", 30, "AC"),
    ("มีเครื่อง/GPU เดิมใช้ได้มั้ย", 30, "AA"),
    ("แชร์เครื่องกับ use case อื่นได้มั้ย", 34, "AD"),
...
ex2 = [1, "Auto Inspection ตรวจคุณภาพชิ้นงานด้วย Machine Learning", "Production",
       ...
       "ไม่เกี่ยวข้อง - ไม่ใช่งานค้นหาเอกสาร", "2 - 3 จุด",
       "1 - 5 MP", "ไม่เกี่ยวกับภาษา",
       "Real-time (0 - 2 วินาที)", "Business Hour (8:00 - 18:00)",
       "ต้องมีเครื่องที่หน้างาน - ติดข้างเครื่องจักร/สายพาน",
       "ต้อง Fine-tune ด้วยข้อมูลบริษัท",
       "ทุกไตรมาส",
       "ไม่มีเลย - ต้องซื้อใหม่ทั้งหมด",
       "แชร์ไม่ได้ - ต้องมีเครื่องของตัวเอง",
       "2 - 3 ปี",
